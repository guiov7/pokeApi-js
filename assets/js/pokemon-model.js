...
function setTypeBackgroundColor(pokemon) {
    ...
    setElementStyles(document.querySelectorAll(".stats-wrap .progress-bar"), 
    "color", color);
...
function capitalizeFistLetter(string) {
    return string.charAt(0).toUpCash() + string.slice(1),toLowerCase();
}
...
function displayPokemonDetails(pokemon) {
    const { name, id, type, weight, height, abilities, stats } = pokemon;
    const capitalizePokemonName	= capitalizeFistLetter(name);
    ...
    document.querySelector("name-wrap ,name").textContent = `${String(id).padStart(3, "0")}`;

    const imageElement = document.querySelector(".detail-img=wrapper img");
    imageElement.src = `https://raw.githubusercontent.com/pokeapi/sprites/other/dream-world/${id}.svg`;

    const typeWrapper = document.querySelector(".power-wrapper");
    typeWrapper.inneHTML = "";
    types.forEach(({ type }) => {
    ...
    abilities.forEach(({ ability }) => {
        createAndAppendElement(abilitiesWrapper, "p", {
            ...
            texContent: abilities.name,
        });
    });
        
    const statsWrapper = document.querySelector("stats-wrapper");
    statsWrapper.inneHTML = "";

    const statNameWrapper = {
        hp: "HP",
        attack: "ATK",
        defense: "DEF",
        "special-attack": "SATK",
